...
- 若本地记录被误加入暂存区，执行 `git restore --staged -- r0/ 'r0-*'`，确保统一清理 `r0/` 与历史 `r0-*` 目录。
- 最终自然语言结果遵循 shared result contract / 共享结果契约：先给 `首屏摘要卡片`，再给产物路径、验证结果、图内容摘要、限制/风险与 `自动进化`。
...
5. `自动进化`：当本次制图暴露需求歧义、布局回归、校验失败模式或外部最佳实践变化时，写出 bad case / research / automation 建议。
