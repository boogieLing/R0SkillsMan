...
- The local record directory for this skill is `./r0/read/`.
- Every substantial reading task MUST leave a local reading record under `./r0/read/`.
...
- Final output MUST start with the unified `首屏摘要卡片`, and its title MUST use `随机颜表情 + 本次需求总结`.
...
- Include an `自动进化` section when the reading exposed repeated confusion, missing docs, or external-practice gaps.
