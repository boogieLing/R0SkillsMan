...
| `r0-ios-agents` | iOS 任务总编排器，用于端到端交付、多 skill 协调、并行 lane 编排，以及 Figma 到 SwiftUI 的阶段化落地。 |
| `r0-question` | 中文知识问答与问题分析技能，强调分点和结构化表达。 |
| `r0-read` | 只读型代码阅读技能，用结构化方式理解现有项目、入口、数据流和关键控制路径。 |
| `r0-request` | 把自然语言需求转换为结构化提示词 DSL，并落盘到 `./r0/request/` 作为本地需求文档。 |
| `r0-review` | 结构化代码审查技能，用于 review、风险分析、回归检查，以及沉淀可复核的本地审查记录。 |
| `r0-roadmap` | 阅读目标目录/仓库的架构与功能流，生成 `./r0/roadmap/` 文档，并在存在时同步摘要到全局 `AGENTS.md`。 |
| `r0-skill-man` | 本地 skill 生态治理技能，用于清理废弃 skill、统一契约、收敛重复能力和做日常维护。 |
| `r0-submit` | 提交与推送安全收尾技能，用于 scope 校验、review-before-push、提交记录和安全调用 `r0push`。 |
| `r0-work` | 长时间工程实施技能，强调范围锁定、控制循环、编译验证、自动修复和生产级代码实现。 |
| `r0-write-lark` | 将本地文章目录同步到 Lark/飞书云文档；当前稳定能力以同步前检查、素材清单、阻塞项归纳和可执行同步计划为主。 |
